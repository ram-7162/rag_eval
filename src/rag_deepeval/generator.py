...
There are two entry points:
  - generate(query, context)        -> returns a validated AnswerSchema.answer
                                        string, parsed via PydanticOutputParser
  - generate_stream(query, context) -> yields the answer in plain-text chunks
                                       as it is produced, for streaming UIs and
                                       for measuring time-to-first-token (TTFT)

Note: generate() uses structured (Pydantic-validated) output because Groq's
chat models don't reliably terminate/format well with a bare StrOutputParser
for this use case. generate_stream() intentionally uses a plain string chain
instead — you cannot validate partial JSON mid-stream, so structured parsing
and token streaming are mutually exclusive. Both chains share the same model,
temperature, and underlying instructions.
"""
import os
from langchain_groq import ChatGroq
from langchain_core.prompts import ChatPromptTemplate
...
class AnswerSchema(BaseModel):
    """Schema for the grounded answer returned by the generator."""
    answer: str = Field(
        description=(
            "The full answer to the student's question, written in flowing "
            "conversational prose, grounded ONLY in the provided course "
            "context. If the context is insufficient, this field must be "
            "exactly: 'I don't have enough information in the course "
...
# --- structured (non-streaming) prompt: adds the JSON format instructions ---
structured_prompt = ChatPromptTemplate.from_template(
    BASE_RULES
    + """

Respond with ONLY a single valid JSON object matching this schema — no
...
# --- plain-text (streaming) prompt: identical rules, no JSON wrapping ---
...
def generate_stream(query: str, context: list[str]):
    ...
    Uses the plain-string chain (not the Pydantic-parsed one) because JSON
    output can't be meaningfully validated or displayed mid-stream — you'd
    either show raw, half-formed JSON to the user or have to buffer the
    whole response anyway, defeating the purpose of streaming.
    ...
    """
    context_text = "\n\n".join(context)
    for chunk in stream_chain.stream({"question": query, "context": context_text}):
        if chunk:                      # skip empty leading chunks
            yield chunk
...
if __name__ == "__main__":
    ...
    # non-streaming (structured / Pydantic-validated)
    print(generate("what is online eval?", ctx))
